...
API de Corrección Formativa con IA diseñada para asistir al profesorado de ESO y Bachillerato en la Comunidad Autónoma de Galicia, iniciando con Filosofía como módulo piloto. El diseño incorpora el marco pedagógico de la LOMLOE y los Decretos 156/2022 y 157/2022 de la Xunta de Galicia, junto con medidas técnicas de supervisión humana, seudonimización y trazabilidad orientadas a los principios del RGPD, AI Act y ENS.

El proyecto no declara certificación ni conformidad legal completa: las medidas implementadas deben validarse en cada contexto de despliegue y uso.
...
*   **AI Safety & Guardrails:** Arquitectura multinivel (*Multilayered Guardrails*) diseñada para reducir salidas no estructuradas y riesgos operativos mediante contratos JSON estrictos (`Pydantic`), reglas de exclusión NEAE y seudonimización de datos. Las validaciones deterministas delimitan la salida del LLM y mantienen la decisión final en manos del docente (`[D-063]`, `[D-002]`).
...
*   **Zero Data Retention (objetivo de diseño):** La PWA aplica una estrategia de minimización de datos y redacción local previa al envío. Mediante la Canvas API, el docente puede censurar identificadores visibles antes de remitir el archivo al backend (`[D-034]`). El flujo está diseñado para que la imagen enviada al procesamiento remoto no contenga PII y para no conservar copias locales innecesarias; la revisión y confirmación final antes de subir el archivo siguen siendo responsabilidad humana.
*   **Resiliencia y Optimización FinOps:** El motor configurado para texto y visión es OpenAI (`gpt-4o-mini`) (`[D-053]`). La integración emplea *Structured Outputs* y validación del contrato `EvaluacionIA`; la cualitativa de ESO se asigna de forma determinista en el backend (`[D-052]`), sin delegarla en el criterio del LLM.
...
Este proyecto documenta el uso de herramientas de IA durante el desarrollo y adopta controles de gobernanza *Human-in-the-Loop*. Esta sección describe medidas técnicas y de transparencia del proyecto; no constituye una declaración de certificación ni de conformidad jurídica completa con el RGPD, el AI Act o el ENS.
