...
| Alembic (migraciones reales) | ✅ mencionada en ADR/backlog | ⚠ parcial | ⚠ pendiente tests específicos | ✅ backlog nota de deuda | Parcial | Falta crear y probar migraciones para columnas nuevas (`estado_feed_forward`, `audit_metadata`). |
| Permisos 403 en feed-forward | ✅ previsto en backlog (`[v0.2-008]`) | ⚠ parcial | ⚠ pendiente test 403 con segundo profesor | ⚠ README menciona auth genérica | Parcial | Endpoint ya verifica propietario, pero falta cobertura de tests 403 y documentación más precisa. |
| Flujo principal de evaluación (`evaluation_router`, REVIEW/GRADED, HitL) | ⚠ parcial (D-024) | ⚠ parcial | ⚠ cobertura no revisada en esta sesión | ⚠ doc no revisada hoy | Parcial | Diseñado en parte, pero pendiente revisión sistemática de estados, decisiones HitL y trazabilidad. |
